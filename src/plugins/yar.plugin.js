 ...
 *
 * > TL;DR; we only use the session we create with **yar** for 'flash' values in support of banner notifications.
 *
 ...
 *
 * We _do not_ intend to use **yar** in these situations. We persist journeys using our `SessionModel`. Most session
 * managers like **yar** rely on creating a cookie. This will hold a session ID. Out-of-the-box they will also support
 * you adding the rest of your session data to the cookie. But as it is common to 'blow' the cookie with too much data
 * they will also support working with server-side storage.
 *
 ...
 *
 * **yar** will store the message in the cookie. The call in the GET not only retrieves the value it also tells Yar to
 * delete it from the cookie. If the user hits refresh the second GET will find `banner` is `null`.
 *
 * ## Multiple sessions
 *
 ...
 *
 * So, we create our own session cookie. The upside is we're not bound by the legacy cookie and when the time
