 ...
 *
 * @param {function} authService - Function provided by the parent app to validate the user is authenticated
 *
 * @returns {object} The Auth plugin object
 */
export default function authPlugin(authService) {
  return {
    name: 'authentication',
    register: async (server, _options) => {
      server.auth.strategy('session', 'cookie', {
        ...
        validate: async (request, session) => {
          return authService(request, session)
        }
      })
